...
func minimumBribesImpl(q []int32) string {
	...
	// 	s := i + 1
	// 	if int32(s) != p {
	// 		bribers = append(bribers, i)
	// 	}
	// }

	// Check how many times each person bribed.
	possibleSits := genPossibleSituations(bribers)
	bribeNums := []map[int]int{}
	for _, s := range possibleSits {
		bribeNum := countBribeNum(genNoBribeQ(len(qInt)), qInt, s)
		bribeNums = append(bribeNums, bribeNum)
	}
	// Filter chaotic patterns.
	nonChaoticBribeNums := []map[int]int{}
	for _, n := range bribeNums {
		if !isChaotic(n) {
			nonChaoticBribeNums = append(nonChaoticBribeNums, n)
		}
	}

	// If there's no non-chaotic situation, return.
	if len(nonChaoticBribeNums) == 0 {
		return "Too chaotic"
	}

	// Get the minimum number of bribing.
	bribeSums := []int{}
	for _, bribeNum := range bribeNums {
		bribeSums = append(bribeSums, sumValMap(bribeNum))
	}

	sort.Slice(bribeSums, func(i, j int) bool { return bribeSums[i] < bribeSums[j] })

	return strconv.Itoa(bribeSums[0])
}
